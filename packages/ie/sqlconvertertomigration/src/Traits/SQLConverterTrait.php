...
trait SQLConverterTrait
{
    ...
    public function convertSQLToMigrationFiles($new_patch)
    {
        $result = $this->extractQueriesOnly(str_replace('"', '', $new_patch), 'SQL_DOWN = u');
        $result_as_array = explode(";", $result);
        $filtered_results = $this->filterResults($result_as_array);
        $filtered_results_according_table = $this->filteredResultsAccordingTable($filtered_results);
        $this->executeOperationForEveryTable($filtered_results_according_table);
    }

    function extractQueriesOnly($string, $needle): string
    {
    ...
    public function filterResults($result_as_array): array
    {
        $filtered_array = [];
    ...
    public function filteredResultsAccordingTable($result_as_array): array
    {
        $queries_of_table = [];
        $tables_name = [];
    ...
    public function executeOperationForEveryTable($filtered_results_according_table)
    {
        $index = 0;
        foreach ($filtered_results_according_table as $tableWithQuery) {
            foreach ($tableWithQuery as $j => $everyQuery) {
            $operation = $this->getOperation($everyQuery);
            if ($operation == 'create') {
                $this->createMigrationForNewTable($everyQuery, $index++);
            }
            else if ($operation == 'drop') {
                $this->dropMigration($everyQuery, $index++);
            }
                ...
                $data['operation'] = 'alter';
                    $file = $this->createFileForMigration($data, $index++);
                    $this->extractDataOfQuery($filtered_results_according_table[$table_name], $file);
                    break;
                }
            }
        }
    ...
    function getOperation($string): string
    {
        $operation = '';
    ...
    private function createMigrationForNewTable(string $query, $index)
    {
        $data = [];
        ...
        $cols_as_array = explode(',', $this->getColsOfCreateOperation($query));
        foreach ($cols_as_array as $col) {
            ...
            if (count($results[0]) > 0) {
                ...
                if (!in_array($col_name, $is_added)) {
                    ...
                    $inner_col['type'] = $type;
                    $attributes = [];
                    if ($this->isPrimaryKey($cols_as_string, $col_name)) {
                        array_push($attributes, 'PRIMARY KEY');
                    }
                    if ($this->isUnique($cols_as_string, $data['table'], $col_name)) {
                        array_push($attributes, 'PRIMARY KEY');
                    }
                    $all_attributes = array_merge($attributes, $this->getAttributes($col));
        ...
        foreach ($data['columns'] as $column) {
            ...
            if (in_array('AUTO_INCREMENT', $column->attributes)) {
                ...
                $method_type = $this->typeConverter($column->type);
                $attributes_assigned = $this->attributeConverter($column->attributes, $data['table'], $column->name, 'create');
        ...
        $line_to_add = $this->getEditableLine($file, '//end_add_columns');
        $this->fileEditContents($file, $line_to_add - 1, implode("\n", $created_cols));
        $line_to_reverse = $this->getEditableLine($file, ' //end_reverse');
        $table_name = $data['table'];
        $reverse_migration = "         Schema::dropIfExists('$table_name');";
        $this->fileEditContents($file, $line_to_reverse - 1, $reverse_migration);
    }

    private function getColsOfCreateOperation($query): string
    {
        $query = str_replace('`', '"', $query);
        $start = strpos($query, '(');
        $close_bracket_exist = [];
    ...
    private function isPrimaryKey($query, $col): bool
    {
    ...
    private function isUnique($query, $table, $col): bool
    {
    ...
    function getAttributes($string): array
    {
        $options = [];
    ...
    public function createFileForMigration(array $data, $index): string
    {
        ...
        if ($data['operation'] == 'drop') {
            ...
            $this->ReplaceWordInFile($file, 'table_name', $data['table']);
        }
        if ($data['operation'] != 'drop') {
            $this->ReplaceWordInFile($file, 'operation', $operation);
        }
        $this->ReplaceWordInFile($file, 'table_name', $data['table']);
        $this->ReplaceWordInFile($file, 'MigrationName', $this->renameMigration($main_name));
        return $file;
    }

    public function ReplaceWordInFile($file, $needle, $replacement)
    {
    ...
    public function renameMigration($name): string
    {
    ...
    public function typeConverter($column_type)
    {
        $types = [];
        // start int
        $types['int'] = 'integer';
    ...
    public function attributeConverter($attributes, $table_name, $col_name, $operation,$flag=null): string
    {
        ...
        if (count($default_value_as_array) > 0) {
            $default_value = $default_value_as_array[0];
            if (isset($default_value)) {
                $default = $this->getDefaultValue($default_value);
    ...
    public function getEditableLine($file, $searchable): int
    {
        $line = 1;
        $handle = fopen($file, "r");
        if ($handle) {
    ...
    public function fileEditContents($file_name, $line, $new_value)
    {
    ...
    function dropMigration($query, $index)
    {
        $data = [];
        $data['operation'] = 'drop';
        $query = str_replace('`', '"', $query);
        ...
        //qu
        $file = $this->createFileForMigration($data, $index);
        $data = $this->getTableInformationBeforeDrop($data['table']);
        $line_to_reverse_drop = $this->getEditableLine($file, '//end_add_columns');
        $created_cols = [];
        foreach ($data['columns'] as $column) {
            $column = (object)$column;
            if (in_array('AUTO_INCREMENT', $column->attributes)) {
                ...
                $method_type = $this->typeConverter($column->type);
                $attributes_assigned = $this->attributeConverter($column->attributes, $data['table'], $column->name, 'drop');
                if ($attributes_assigned != "") {
                    $end = $attributes_assigned;
                } else {
                    $end = ';';
        ...
        $this->fileEditContents($file, $line_to_reverse_drop - 1, implode("\n", $created_cols));
    }

    private function getTableInformationBeforeDrop($table): array
    {
    ...
    function extractDataOfQuery($queries_as_array, $file)
    {
        foreach ($queries_as_array as $mysql){
            ...
            $attributes = [];
            if ($this->isPrimaryKey($mysql_filter, $data['column'])) {
                array_push($attributes, 'PRIMARY KEY');
            }
            $all_attributes = array_merge($attributes, $this->getAttributes($mysql_filter));
            $data['attributes'] = $all_attributes;
            $data = (object)$data;
            $method_type = $this->typeConverter($data->type);
            $attributes = $data->attributes;
            $attributes_assigned = $this->attributeConverter($attributes, $data->table, $data->column, $data->operation);
            if ($attributes_assigned != '') {
                $end = $attributes_assigned;
            } else {
                $end = ';';
            ...
            $col_migration .= "\n";
            $line_to_add = $this->getEditableLine($file, '//end_add_columns');
            $this->fileEditContents($file, $line_to_add - 1, $col_migration);
            //que
            if (Schema::hasColumn($data->table, $data->column)) {
                $result = $this->getTableInformationBeforeDrop($data->table);
                foreach ($result['columns'] as $one_col) {
                    $one_col = (object)$one_col;
                    if ($one_col->name == $data->column) {
                        $type = $this->typeConverter($one_col->type);
                        $attribute_as_migration = '';
...
//                                }
//                            }
                            $attribute_as_migration .= $this->attributeConverter($one_col->attributes,$data->table,$data->column,$data->operation,'reverse');
                        }
                        $line_to_drop = $this->getEditableLine($file, '//end_reverse');
                        $full_table_name = "'" . $data->table . "'";
                        $reverse_migration = '
            $table->' . $type . '("' . $data->column . '")' . $attribute_as_migration . '';
                        $reverse_migration .= "\n";
                        $this->fileEditContents($file, $line_to_drop - 1, $reverse_migration);
                    }
                }
